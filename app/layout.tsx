...
/**
 * iOS launch screens, one per supported iPhone. iOS will only use an image
 * whose media query matches the device exactly, which is why this is a list
 * rather than one file. Portrait only — a phone launched in landscape falls
 * back to the plain background, which is rare enough not to double the assets.
 * Images live in /public/splash and are regenerated by scripts/gen-splash.mjs.
 */
const APPLE_SPLASH = [
  { w: 430, h: 932, dpr: 3, href: '/splash/splash-1290x2796.png' },
  { w: 402, h: 874, dpr: 3, href: '/splash/splash-1206x2622.png' },
  { w: 393, h: 852, dpr: 3, href: '/splash/splash-1179x2556.png' },
  { w: 428, h: 926, dpr: 3, href: '/splash/splash-1284x2778.png' },
  { w: 390, h: 844, dpr: 3, href: '/splash/splash-1170x2532.png' },
  { w: 375, h: 812, dpr: 3, href: '/splash/splash-1125x2436.png' },
  { w: 414, h: 896, dpr: 2, href: '/splash/splash-828x1792.png'  },
  { w: 414, h: 896, dpr: 3, href: '/splash/splash-1242x2688.png' },
  { w: 375, h: 667, dpr: 2, href: '/splash/splash-750x1334.png'  },
];
...
export const metadata: Metadata = {
  ...
  icons: {
    ...
    // No `shortcut: ['/favicon.ico']` — that file does not exist, so every page
    // load fetched it and took a 404. The PNG + SVG icons above already cover
    // every browser that would have asked for it.
  },
  appleWebApp: {
    capable: true,
    title: SITE_NAME,
...
export const viewport: Viewport = {
  // Two entries: the status bar / browser chrome follows the app's theme
  // instead of staying brand-blue on a dark screen.
  themeColor: [
    { media: '(prefers-color-scheme: light)', color: '#1257E5' },
    { media: '(prefers-color-scheme: dark)',  color: '#0C1020' },
  ],
  width:        'device-width',
  initialScale: 1,
  // maximumScale is deliberately NOT set. It used to be 1, which suppressed
  // iOS's zoom-on-focus but also blocked pinch-zoom for everyone — a WCAG
  // 1.4.4 failure. Inputs are 16px on touch devices now (globals.css), which
  // removes the auto-zoom at its source and lets people zoom again.
  ...
  return (
    <html lang="en" className={`${rubik.variable} ${nunitoSans.variable}`}>
      <head>
        ...
        {/* iOS launch screens.
            Android builds a splash from the manifest; iOS does not, so without
            these the installed app shows a blank WHITE screen from tap until
            React boots — the most "this is just a website" moment there is,
            and it happens on every launch. iOS matches on exact device
            metrics, so each size needs its own tag; anything unmatched simply
            falls back to the old white, so this can only improve.
            Regenerate with scripts/gen-splash.mjs. */}
        {APPLE_SPLASH.map(s => (
          <link
            key={s.href}
